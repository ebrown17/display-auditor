...
public enum StationPlatformAndViewCache {

	INSTANCE;

	private StationPlatformAndViewCache() {
		buildPlatformCache();
		buildStationCache();
		buildViewCaches();
	}
	...
	private void buildStationCache() {
		for (Platform platform : platformCache.values()) {
	...
	private void buildViewCaches(){
		for (Map.Entry<String, ArrayList<Platform>> station : stationCache.entrySet()) {
