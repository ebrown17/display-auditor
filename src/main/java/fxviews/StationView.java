...
public class StationView {
  ...
  private void buildStationView() {
    ...
    stationView.setStyle("-fx-background-color: skyblue ; -fx-border-color: black;");
    // stationView.setGridLinesVisible(true);
    JavaFxObservable.eventsOf(stationView, MouseEvent.MOUSE_ENTERED)
        .map(me -> "-fx-background-color: f08080; -fx-border-color: black;").observeOn(JavaFxScheduler.platform())
        .subscribe(stationView::setStyle);

    JavaFxObservable.eventsOf(stationView, MouseEvent.MOUSE_EXITED)
        .map(me -> "-fx-background-color: skyblue; -fx-border-color: black;").observeOn(JavaFxScheduler.platform())
        .subscribe(stationView::setStyle);
